...
def year_controls(dataset: pd.DataFrame) -> tuple[pd.DataFrame, tuple[int, int]]:
...
def render_kmeans(dataset: pd.DataFrame) -> None:
    dataset, years = year_controls(dataset)
    st.sidebar.subheader("Clustering")
    ...
    render_metrics(records=len(clustered), grid_cells=None, groups=clustered["Cluster"].nunique(), noise_percent=None, years=years)
